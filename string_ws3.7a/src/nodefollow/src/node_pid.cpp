...
void NodePID::check_obs(double danger_dis_f, double danger_dis_fl, double danger_dis_fr)
{
...
void NodePID::tofCallback(const nlink_parser::TofsenseCascade::ConstPtr &msg_tof)
{
    ...
    check_obs(danger_dis_f, danger_dis_fl, danger_dis_fr);
...
void NodePID::messageCallback(const nlink_parser::LinktrackAoaNodeframe0::ConstPtr &msg_aoa)
{
    ...
    //是否到达目标点
    if ((msg_aoa->nodes[0].dis - dis_tar) > tolerance)
        if_achieve = false;
    else if ((msg_aoa->nodes[0].angle - ang_tar) > toleranceA)
        if_achieve = false;
    else
        if_achieve = true;
